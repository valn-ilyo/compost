  ...
  () => {
    ...
    function reconcile(): void {
      ...
      const lostHabits = activeHabits.value.filter((h) => {
        ...
        // Mirror streak walker rule 4: pause windows are transparent gaps.
        // A habit paused yesterday and resumed today re-enters activeHabits with
        // status 'active', so it reaches this filter — but yesterday was covered
        // by a pause window and must not be treated as a missed day.
        if (isPausedOnDate(h.templateId, yesterday)) return false;
