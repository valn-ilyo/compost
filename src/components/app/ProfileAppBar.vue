...
async function deleteAccount() {
  ...
  try {
    ...
    // On success deleteAccountAndLogout() navigates to /auth, unmounting this
    // component. The finally block below still runs but is a harmless no-op on
    // the dead instance.
  } catch {
    // Only reachable if the delete_account() RPC itself fails (account is still
    // intact). Post-RPC steps (resetAllStores, signOut, router.push) are
    // extremely unlikely to throw and are not retryable here.
    notify({
