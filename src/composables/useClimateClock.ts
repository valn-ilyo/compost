...
import type { ClockModule, NewsfeedItem, Lifeline, ClockDisplay, ParsedUnit } from '@/types/app.types'
...
function hydrate(modules: Record<string, ClockModule>) {
  ...
  lifelines.value   = LIFELINE_KEYS.flatMap(key => modules[key] ? [moduleToLifeline(key, modules[key])] : [])
...
function stopTicks() {
  ...
  if (lifelineTick) { clearInterval(lifelineTick); lifelineTick = null }
}
...
async function fetchAndPersist(store: ReturnType<typeof useClockDataStore>) {
  ...
  const res = await fetch('https://api.climateclock.world/v2/clock.json', { signal: controller.signal })
...
async function init() {
  ...
  if (store.isCacheValid && store.deadlineTs) {
...
export function useClimateClock() {
  ...
  const lifelineDisplay = computed(() => {
    ...
    const dec = lf.resolution < 0.000001 ? 8
              : lf.resolution < 0.01     ? 6
              : lf.resolution < 0.1      ? 2
              : lf.resolution < 1        ? 1 : 0
    return lf.prefix + v.toLocaleString('en-IN', { minimumFractionDigits: dec, maximumFractionDigits: dec })
  })

  return { loading, deadlineLabel, cd, currentLifeline, lifelineDisplay, lifelineIndex, tickerItems, isStale, fetchFailed }
}
